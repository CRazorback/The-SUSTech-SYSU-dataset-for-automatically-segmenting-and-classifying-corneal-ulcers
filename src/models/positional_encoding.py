...
if __name__ == '__main__':
    position_encoder = SinusoidalPositionalEmbedding(embedding_dim=8, padding_idx=0, init_size=512, center_shift=100)
    x = torch.ones((1, 1, 512, 512))
    y = position_encoder(x)
    import imageio
    import numpy as np
    for i in range(y.size()[1]):
        y_np = y[0, i].numpy() * 255
        imageio.imwrite('debug/position_{}.png'.format(i), y_np.astype(np.uint8))